...
function ProjectCard({
  ...
  index: number
  isVisible: boolean
  activeInspiration: number | null
  setActiveInspiration: (v: number | null) => void
  ...
  return (
    ...
    >
      <div className="absolute top-0 left-4 w-[70px] h-[1px] pointer-events-none z-[11]" style={{ background: "linear-gradient(to right, transparent, rgba(255,255,255,0.6), transparent)" }} />
      <div className="absolute top-0 right-4 w-[70px] h-[1px] pointer-events-none z-[11]" style={{ background: "linear-gradient(to right, transparent, rgba(255,255,255,0.6), transparent)" }} />
      <div className="absolute bottom-0 left-4 w-[70px] h-[1px] pointer-events-none z-[11]" style={{ background: "linear-gradient(to right, transparent, rgba(255,255,255,0.6), transparent)" }} />
      <div className="absolute bottom-0 right-4 w-[70px] h-[1px] pointer-events-none z-[11]" style={{ background: "linear-gradient(to right, transparent, rgba(255,255,255,0.6), transparent)" }} />
      <div className="absolute inset-0 rounded-2xl pointer-events-none z-10" style={{ boxShadow: "inset 2px 2px 3px 0 rgba(255, 255, 255, 0.5), inset -1px -1px 2px 1px rgba(255, 255, 255, 0.5)" }} />
      <div className="absolute inset-0 rounded-2xl pointer-events-none z-10 opacity-60" style={{ boxShadow: "inset 1px 1px 2px 0 rgba(255, 255, 255, 0.3), inset -2px -2px 3px 2px rgba(255, 255, 255, 0.2)" }} />
      ...
      <div className="relative p-5 md:p-8 z-30 flex flex-col h-full">
        <div className="flex items-start justify-between gap-3">
          <h3 className="text-lg md:text-3xl font-bold mb-2 md:mb-3 text-white transition-all" style={{ textShadow: "0 2px 8px rgba(0, 0, 0, 0.9)" }}>
            {project.title}
          </h3>
          <div
          ...
          >
            ...
            <div className={`absolute right-0 top-full mt-2 w-72 ${activeInspiration === index ? "opacity-100 scale-100 pointer-events-auto" : "opacity-0 scale-95 pointer-events-none"} transition-all duration-200 origin-top-right z-50`}>
              <div className="relative backdrop-blur-2xl bg-black/90 border border-white/20 rounded-xl p-4 shadow-[0_8px_32px_0_rgba(0,0,0,0.7)]">
                <div className="absolute inset-0 pointer-events-none rounded-xl" style={{ boxShadow: "inset 1px 1px 2px 0 rgba(255, 255, 255, 0.2), inset -1px -1px 2px 0 rgba(255, 255, 255, 0.1)" }} />
                <div className="absolute top-0 left-4 w-[40px] h-[1px] pointer-events-none" style={{ background: "linear-gradient(to right, transparent, rgba(255,255,255,0.4), transparent)" }} />
                <div className="relative z-10">
                  <div className="flex items-center gap-1.5 mb-2">
                    <Lightbulb className="w-3.5 h-3.5 text-amber-300" />
                    <span className="text-xs font-semibold text-amber-300 uppercase tracking-wider">Inspiration</span>
                  </div>
                  <p className="text-slate-200 text-sm leading-relaxed" style={{ textShadow: "0 1px 4px rgba(0,0,0,0.8)" }}>
                    {project.inspiration}
                  </p>
                </div>
              </div>
        ...
        <p className="text-slate-200 leading-relaxed mb-5 text-sm md:text-base" style={{ textShadow: "0 1px 4px rgba(0, 0, 0, 0.8)" }}>
          {project.description}
        </p>

        <div className="flex flex-wrap gap-2 mb-6">
          {project.tech.map((t) => (
            <span key={t} className="px-3 py-1 text-xs md:text-sm bg-white/10 rounded-full border border-white/20 text-white/90 hover:bg-white/20 hover:border-white/40 transition-colors" style={{ textShadow: "0 1px 4px rgba(0, 0, 0, 0.8)" }}>
              {t}
            </span>
          ))}
        </div>
        ...
        <div className="mt-auto">
          ...
          >
            <div className="absolute inset-0 rounded-full pointer-events-none z-10" style={{ boxShadow: "inset 2px 2px 3px 0 rgba(255, 255, 255, 0.5), inset -1px -1px 2px 1px rgba(255, 255, 255, 0.5)" }} />
            <div className="absolute inset-0 rounded-full pointer-events-none z-10 opacity-70" style={{ boxShadow: "inset 1px 1px 2px 0 rgba(255, 255, 255, 0.3), inset -2px -2px 3px 2px rgba(255, 255, 255, 0.2)" }} />
